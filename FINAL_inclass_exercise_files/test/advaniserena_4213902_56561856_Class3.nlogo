...
to setup
  clear-all
  ask patches [
    ;; create approximately spawn-percentage% alive patches
...
to basic2
  ...
  ask patches [
    ...
    ;; each patch counts its number of yellow neighboring patches
    ;; and stores the value in its live-neighbors variable
    set live-neighbors count neighbors with [pcolor = yellow]]
  ask patches [
      if live-neighbors = 4 [set plabel "dead"]
      if live-neighbors > 4 [set plabel "dead"]
      if live-neighbors >= 4 [set pcolor red]
    ]
end
