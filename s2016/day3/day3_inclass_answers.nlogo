...
### 2
Write a function that clears the world and makes a glider with its top-left corner at (0,0) which moves down and to the right (see pg. 64).
### 2
Find a still-life configuration with five live cells and write a functions which clears the world and creates it in the middle.
### 3
Find a period two oscillator different from the blinker and write a function which creates it at the origin.
### 4
Write a function which implements an altered set of transition rules. If a patch is set to die in the old rules, instead of changing its pcolor to blue, change it to yellow. All yellow patches should die at the beginning of each tick.
## B
### 1
Write a function which creates the glider gun shown on page 65 with top-left corner at (-25, 25).
### 2
Create four sliders, min-spawn, max-spawn, min-die, and max-die, which scale between 0 and 8. Write a new go function on the model of the basic one, except replacing the hard-coded numbers with slider values. 
### 3
@#$#@#$#@
default
true
